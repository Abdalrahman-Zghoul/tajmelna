...
<div class="card">
    ...
    <div class=" row card-body">
        <div class="col-md-4">
          <form method="POST" action="{{ route('update-saloon',$saloon->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
        ...
        <div class="col-md-4">
            ...
            <div class="form-group">
                ...
                <input type="file" id="image{{$counter+1}}" class="btn ms-2 text-start border d-block" name="image{{$counter+1}}">
              </div>
              <?php $counter+=1 ?>
            @endfor
            {{-- @for ($i = $counter+1; $i <= 4; $i++)
